...
- Imported map and video assets are copied into campaign-owned asset folders before being referenced by metadata. Bulk scene import uses the same asset import path and creates one scene per selected map file. Imported token art is cropped into a square token-ready campaign asset; the original oversized token source file is not retained after the crop is confirmed. Thumbnail generation is best-effort; missing thumbnails should not make campaign metadata invalid.
